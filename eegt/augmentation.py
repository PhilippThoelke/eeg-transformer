...
def rescale(x, ch_pos, mask, max_strength=0.2):
...
def rescale_channel_pos(x, ch_pos, mask, max_strength=0.05):
...
def random_mean(x, ch_pos, mask, max_strength=0.075):
...
def random_mean_channel_pos(x, ch_pos, mask, max_strength=0.05):
...
def shift_samples(x, ch_pos, mask, max_n=8):
...
def resample(x, ch_pos, mask, max_change_pct=20):
...
def temporal_dropout(x, ch_pos, mask, max_samples=32, prob=0.15):
...
def channel_dropout(x, ch_pos, mask, min_rate=0.1, max_rate=0.5):
...
def randomize_phase(x, ch_pos, mask, strength=0.3):
